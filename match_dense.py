    ...
    with h5py.File(str(match_path), "a") as fd:
        for data in tqdm(loader, smoothing=0.1):
            ...
            for i in range(len(name0)):
                n0 = name0[i]
                n1 = name1[i]
                if n0 in existing_refs:
                    img_pairs.append((image1[i], image0[i]))
                    flip_flags.append(True)
                else:
                    img_pairs.append((image0[i], image1[i]))
                    flip_flags.append(False)

            batch_preds = model.match_batch(img_pairs, batch_size=batch_size)
