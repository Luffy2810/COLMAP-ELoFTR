...
def compute_triangulation_angles(x1, x2, cam_pose):
    """
    Compute triangulation angles between rays from two spherical cameras.
    Handles far away points by considering the acute angle.
    ...
    C1 = np.array([0, 0, 0])
    C2 = cam_pose[0:3, 3]
    baseline = C2 / np.linalg.norm(C2)  # Normalized baseline direction
    ...
    # Get acute angles: if angle > 90°, use 180° - angle
    # This ensures we get the acute angle between rays
    angles = np.arccos(np.clip(dots, -1.0, 1.0))
    obtuse_mask = angles > np.pi/2
    angles[obtuse_mask] = np.pi - angles[obtuse_mask]
    
    # For acute angles, also check angle with baseline
    # This helps filter out far away points
    baseline_dots1 = np.abs(np.einsum('ij,j->i', x1_world, baseline))
    baseline_dots2 = np.abs(np.einsum('ij,j->i', x2_world, baseline))
    
    # Filter points where rays are too parallel to baseline
    parallel_to_baseline = (baseline_dots1 > 0.95) | (baseline_dots2 > 0.95)
    angles[parallel_to_baseline] = 0  # These will get filtered by min angle check
...
class RANSAC_8PA:
    ...
    def __init__(self):
        ...
        self.min_triangulation_angle = 2
    ...
    def run(self, bearings_1, bearings_2):
        ...
        if self.best_inliers is not None and np.sum(self.best_inliers) >= 8:
            ...
            tri_angles = compute_triangulation_angles(best_bearings_1, best_bearings_2, cam_pose)
            ...
            final_inliers = (np.abs(sample_residuals) < self.residual_threshold) & angle_mask
            
            # Create a new mask for all original points
            updated_inliers = np.zeros(bearings_1.shape[1], dtype=bool)
            # Only update the inliers that were previously considered inliers
            updated_inliers[self.best_inliers] = final_inliers
            
            # Update all the best values
            self.best_inliers = updated_inliers  # Add this line
